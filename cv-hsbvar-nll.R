# cv-hsbvar-nll.R
# Interpolation-based cross-validation for H-SBVAR with NLL loss.
#
# Methodology mirrors cv-hsbar.R:
...
#   3. Fit H-SBVAR on the thinned training set via hsbvar(keep_rows=...).
...
#' Interpolation CV for H-SBVAR (NLL loss) over a lambda path
#'
...
#' @param ...               Additional arguments forwarded to \code{hsbvar()}.
#'
#' @return List: cv_table (lambda, mspe, mspe_se), best, val_points, keep_rows.
cv_hsbvar_nll <- function(Y,
                          p = 1L,
                          lambda = NULL,
                          c_scale = 1,
                          val_spacing = 10L,
  ...
  for (j in seq_len(n_lambda)) {
    ln <- lambda_vec[j]
    fit <- tryCatch(
      hsbvar( # nolint: object_usage_linter
        Y, p,
        lambda = ln,
        c_scale = c_scale,
        keep_rows = keep_rows,
...
#'
#' @param cv_result Output of \code{cv_hsbvar_nll()}.
#' @param log_x     Log-scale the lambda axis? Default TRUE.
#' @param ...       Extra arguments passed to \code{plot()}.
plot_cv_hsbvar_nll <- function(cv_result, log_x = TRUE, ...) {
  ...
  plot(x, tbl$mspe,
    ...
    main = "H-SBVAR NLL \u2013 cross-validation MSPE",
    ...
  )
